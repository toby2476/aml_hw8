...
def import_data():
	...
	return train_images[0:19,:],train_labels[0:19],update_coords,initial_params
...
def save_all(images,description):
...
def main():
	...
	theta_hh = 0.8
	num_iter = 10
	train_images, train_labels, update_coords,initial_params = import_data()
	train_images = binarize(train_images)
	save_all(train_images,'original')
	train_images = add_noise(train_images)
	save_all(train_images,'noisy')
	denoised_images = mean_field_wrapper(update_coords,train_images,initial_params,theta_xh,theta_hh,num_iter)
	save_all(denoised_images,'denoised')
